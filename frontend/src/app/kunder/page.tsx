...
export default function KunderPage() {
  ...
  const kanRedigere = useTilgang(8);
  ...
  const filtered = kunder.filter(k => {
    ...
    return (
      (k.navn.toLowerCase().includes(q) ||
       (k.orgNr ?? '').toLowerCase().includes(q) ||
       (k.epost ?? '').toLowerCase().includes(q)) &&
      (!visKunAktive || k.aktiv)
    );
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editKunde) {
        await put(`/kunder/${editKunde.id}`, form);
        showSuccess('Kunde oppdatert.');
      } else {
        await post('/kunder', form);
        showSuccess('Kunde opprettet.');
      }
      setShowModal(false);
      load();
              ...
              <tr key={k.id}>
                ...
                <td>
                  {kanRedigere && <div style={{ display: 'flex', gap: '0.4rem' }}>
                    ...
                    <button className="btn btn-sm btn-danger" onClick={() => handleDelete(k.id)}>Slett</button>
                  </div>}
                </td>
              </tr>
            ))}
      ...
      {showModal && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowModal(false); }}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2>{editKunde ? 'Rediger kunde' : 'Ny kunde'}</h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-actions">
                ...
                <button type="submit" className="btn btn-primary">{editKunde ? 'Lagre' : 'Opprett'}</button>
              </div>
            </form>
          </div>
        </div>
